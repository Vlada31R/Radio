...
class ViewController: UIViewController {
    ...
    @objc func handleLongPress(_ sender: UILongPressGestureRecognizer) {
        ...
        if let index = indexPath {
            ...
            let alertController = UIAlertController(title: "Station", message:stations[index[1]].name, preferredStyle: .alert)
            ...
            { (action) in
                self.favourites.append(self.stations[index[1]])
            }
            ...
            { (action) in
                self.stations.remove(at: index[1])
